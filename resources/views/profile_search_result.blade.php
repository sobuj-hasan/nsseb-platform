...
@section('title', 'Profile Search Result')
@section('body')
    <header id="header" class="main-wrapper">
        <div class="index-banner py-5">
            <div class="banner-overlay py-lg-4">
                ...
                <div class="row">
                    @foreach ($search_results as $profile)
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                            <div class="profile">
                                <a href="{{ route('profile', $profile->id) }}"><img class="match-img" src="{{ asset('nsseb_assets/media/images/profile_img') }}/{{ $profile->user->profile_photo }}" alt=""></a>
                                <h6>{{ \Carbon\Carbon::parse($profile->birth_date)->diff(\Carbon\Carbon::now())->format('%y years, %m months') }}</h6>
                                <a href="{{ route('profile', $profile->id) }}"><h4>{{ Str::limit($profile->full_name, 20, $end='..') }} </h4></a>
                                <p> <i class="fas fa-map-marker-alt"></i> {{ $profile->present_city }} {{ $profile->country }}</p>
                            </div>
                        </div>
                    @endforeach
                    @empty($profile)
                        <div class="text-danger">Search results not found... </div>
                    @endempty
                </div>
            </div>
        </div>
                        ...
                        @foreach ($top_profile as $profile)
                            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                                <a href="{{ route('profile', $profile->id) }}">
                                    <img class="top-img" src="{{ asset('nsseb_assets/media/images/profile_img') }}/{{ $profile->user->profile_photo }}" alt="">
                                </a>
                                <h6>{{ \Carbon\Carbon::parse($profile->birth_date)->diff(\Carbon\Carbon::now())->format('%y years, %m months') }}</h6>
                                <a style="text-decoration: none;" href="{{ route('profile', $profile->id) }}">
                                    <h4>{{ Str::limit($profile->full_name, 20, $end='..') }}</h4>
                                </a>
                                <p> <i class="fas fa-map-marker-alt"></i> {{ $profile->present_city }} {{ $profile->country }}</p>
                            </div>
                        @endforeach
                ...
                <div class="top-profile-part new-profile-part">
                    <div class="row m-auto">
                        @foreach ($new_profile as $profile)
                            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                                <a href="{{ route('profile', $profile->id) }}"><img class="top-img" src="{{ asset('nsseb_assets/media/images/profile_img') }}/{{ $profile->user->profile_photo }}" alt=""></a>
                                <h6>{{ \Carbon\Carbon::parse($profile->birth_date)->diff(\Carbon\Carbon::now())->format('%y years, %m months') }}</h6>
                                <a href="{{ route('profile', $profile->id) }}"><h4>{{ Str::limit($profile->full_name, 20, $end='..') }}</h4></a>
                                <p> <i class="fas fa-map-marker-alt"></i> {{ $profile->present_city }} {{ $profile->country }}</p>
                            </div>
    ...
    <div id="wedding" class="wedding main-wrapper">
        <div class="container">
            ...
            <div class="row m-auto wedding-shop-slider3">
